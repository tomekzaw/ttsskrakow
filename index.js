...
function parseVehiclesJson(data, category) {
  return data.vehicles
    .filter(
      (vehicle) => !vehicle.isDeleted && vehicle.latitude && vehicle.longitude
    )
    .map((vehicle) => ({
      ...
      category: category,
      line: vehicle.name.split(" ", 1)[0],
    }));
}

app.get("/api/stops", async (req, res) => {
  const endpoint =
    "/internetservice/geoserviceDispatcher/services/stopinfo/stops?left=-648000000&right=648000000&top=324000000&bottom=-324000000";
  const url_A = "http://91.223.13.70" + endpoint;
  const url_T = "http://www.ttss.krakow.pl" + endpoint;

  const [response_T, response_A] = await axios.all([
    axios.get(url_T),
    axios.get(url_A),
  ]);
...
app.get("/api/vehicles", async (req, res) => {
  const url_A =
    "http://91.223.13.70/internetservice/geoserviceDispatcher/services/vehicleinfo/vehicles?positionType=CORRECTED";
  const url_T =
...
app.get("/api/path", async (req, res) => {
  ...
  const baseUrl =
    category === "tram" ? "http://www.ttss.krakow.pl" : "http://91.223.13.70";
  const url = `${baseUrl}/internetservice/geoserviceDispatcher/services/pathinfo/vehicle?id=${vehicleId}`;

  const response = await axios.get(url);

  const path = response.data.paths[0].wayPoints.map((point) => [
    point.lat / 3_600_000,
    point.lon / 3_600_000,
  ]);
